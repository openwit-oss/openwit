...
impl BatchTracker {
    ...
    pub async fn new(database_url: &str) -> Result<Self> {
        ...
        let parsed_url = if !database_url.contains("://") {
        ...
        let db_pool = loop {
            match PgPoolOptions::new()
                .max_connections(10)
                .min_connections(1)  // Start with just 1 minimum connection
                .acquire_timeout(std::time::Duration::from_secs(30))  // Increased timeout
                .connect(parsed_url)
                .await
            {
                Ok(pool) => {
                ...
                Err(e) => {
                    ...
                    if retry_count >= max_retries {
                        ...
                        error!("Please ensure PostgreSQL is running and accessible at: {}", parsed_url);
                        error!("Common issues:");
                        error!("  - PostgreSQL service not running");
                        error!("  - Database 'openwit' does not exist (create with: createdb openwit)");
    ...
    async fn initialize_schema(&self) -> Result<()> {
        ...
        CREATE INDEX IF NOT EXISTS idx_batch_tracker_status ON batch_tracker(kafka_pip, wal_pip, ingestion_pip);
        "#;
    ...
    pub async fn create_batch(
        ...
        debug!(
            "Creating batch record: id={}, client_id={}, size={}, messages={}", 
            batch_id, client_id, batch_size, message_count
        );
        
        let query = r#"
        INSERT INTO batch_tracker (
            batch_id, client_id, kafka_pip, batch_size, message_count,
    ...
    pub async fn update_wal_completed(&self, batch_id: Uuid) -> Result<()> {
        ...
        SET wal_pip = true, 
            ...
            kafka_duration_ms = EXTRACT(EPOCH FROM (NOW() - created_at)) * 1000
        WHERE batch_id = $1
        "#;
    ...
    /// Update kafka pipeline status (batch sent to ingestion)
    pub async fn update_kafka_completed(&self, batch_id: Uuid) -> Result<()> {
        debug!("Updating kafka completed for batch {}", batch_id);
        
        let query = r#"
        UPDATE batch_tracker 
        SET kafka_pip = true, 
            kafka_completed_at = NOW()
        WHERE batch_id = $1
        "#;
        ...
        let rows_affected = sqlx::query(query)
            ...
            .execute(&self.db_pool)
            .await
            .context("Failed to update kafka status")?
            .rows_affected();
        
        if rows_affected == 0 {
            warn!("No batch found with id {}", batch_id);
        } else {
            debug!("Kafka completed for batch {}", batch_id);
        }
    ...
    pub async fn update_ingestion_completed(&self, batch_id: Uuid) -> Result<()> {
    ...
    pub async fn update_storage_completed(&self, batch_id: Uuid) -> Result<()> {
    ...
    pub async fn update_index_completed(&self, batch_id: Uuid) -> Result<()> {
    ...
    pub async fn update_janitor_completed(&self, batch_id: Uuid) -> Result<()> {
    ...
    pub async fn record_error(&self, batch_id: Uuid, stage: &str, error_message: &str) -> Result<()> {
    ...
    /// Get batch status
    pub async fn get_batch_status(&self, batch_id: Uuid) -> Result<Option<BatchRecord>> {
        let query = r#"
        SELECT batch_id, client_id, kafka_pip, wal_pip, ingestion_pip, 
               storage_pip, index_pip, janitor_pip, batch_size, message_count,
               first_offset, last_offset, topics, created_at
        FROM batch_tracker 
        WHERE batch_id = $1
        "#;
    ...
    pub async fn get_stuck_batches(&self, older_than_minutes: i32) -> Result<Vec<Uuid>> {
        let query = r#"
        SELECT batch_id
        FROM batch_tracker
        ...
        let batch_ids: Vec<Uuid> = rows.iter()
